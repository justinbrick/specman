...
- Current prompt surface: `feat`, `ref`, `revision`, `fix`, `spec`, `impl`, `migration`, `compliance`.
  - Prompts are argument-driven templates: each prompt accepts required arguments describing what the user wants (e.g. the specification `request`, implementation/scratch-pad `request`, or migration `codebase`) plus a target locator where applicable, and the server interweaves those argument values directly into the rendered prompt. There is no trailing free-form user-input section; when the supplied arguments are insufficient, the prompt instructs the agent to keep querying the user until the request is unambiguous.
