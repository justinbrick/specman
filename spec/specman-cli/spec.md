...
- Creation commands MUST invoke the SpecMan Core Template Orchestration capabilities defined in [SpecMan Core](../specman-core/spec.md#concept-template-orchestration) to obtain a `TemplateDescriptor` before rendering artifacts. The CLI MUST treat the descriptor (including locator precedence, pointer resolution, and packaged-default fallbacks) as authoritative and MUST NOT reimplement those behaviors locally.
