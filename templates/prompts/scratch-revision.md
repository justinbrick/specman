...
5. Run a conflict audit before drafting changes: scan {{target_spec_path}} and every dependency for statements that would contradict the proposed update, capture each conflict with the original quote, heading, requirement level, and a question that confirms the user’s intent, then flag whether the new change should override, amend, or respect the existing statement.
