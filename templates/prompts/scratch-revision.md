...
2. Copy `templates/scratch/scratch.md`, retain HTML comments until fulfilled, and update the front matter with `target: {{target_path}}`, `branch: {{branch_name}}`, and `work_type: { revision: { revised_headings: [...] } }`, filling `revised_headings` from the revision details in {{arguments}} and recording {{artifact_name_or_request}} in Context if the artifact name requires confirmation.
3. Check out {{branch_name}} before editing the scratch pad further so the revision work proceeds on that branch from the start.
4. Use Context plus Scope & Goals to summarize the requested changes, impacted headings, and acceptance criteria based on {{arguments}}, keeping the prose concise and decision-ready while grounding in {{context}} and {{dependencies}} when relevant.
